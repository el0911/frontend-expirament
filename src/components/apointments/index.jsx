...
import { getAPIS, postAPIS } from '../../utilFunction';

function Shedule(props) {
    const [value, onChange] = React.useState('10:00');
    const [valueD, onChangeD] = React.useState(new Date());
    ...
    const [detail, setData] = React.useState({})
    const load = async () => {
        const { patients } = await getAPIS('patient')
        setP(patients)
    }

    const saveApointment = async (patient) => {

         await postAPIS('apointment',{
            user:patient,
            time:value,
            date:valueD
        })
        ...
        <Component>
            ...
            <div className="sider">
                ...
                <div className="patients">
                    {p.map(({ first_name, last_name, address, lastAccess,_id }) => <Patients onclick={e => {

                        setData({
                            address, lastAccess, first_name, last_name,_id
                        })
                ...
                {detail.first_name &&
                    ...
                    <div>
                        <div className="header">
                            ...
                            <p>{'user Id'}</p>
                            <p>{moment(detail.lastAccess).format('LLL')}</p>
                        </div>
                        ...
                        <div className="records">
                            ...
                            ].map(({ value, key, component }) => <Details detailkey={key} component={component} value={value} />)}
                        </div>

                        <button onClick={e=>{
                            saveApointment(detail._id)
                        }} >
                            Save Appointment
                        </button>
                    </div>
